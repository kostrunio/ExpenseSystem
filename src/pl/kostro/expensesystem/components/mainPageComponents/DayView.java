...
public class DayView extends CustomComponent {
	 ...
	 */
	public DayView(final ExpenseSheet expenseSheet, final Date date, final MonthView parentView) {
		this.date = date;
	...
	private void prepareExpensesLayout() {
		...
		for (int i = 0; i < categoryList.size(); i++) {
			...
			CategoryExpense categoryExpenseMap = expenseSheet.getDateExpenseMap().get(date)
					.getCategoryExpenseMap().get(category);
			...
			catButton.addClickListener(new Button.ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					if (event.getButton().getData() instanceof Category) {
						...
						setCompositionRoot(new CategoryExpenseView(expenseSheet, date, category, parentView));
					}
				}
